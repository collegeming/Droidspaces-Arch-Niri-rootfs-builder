...
- 终端为 **ghostty 优先**（三层保障）：① ALARM 仓库收录后直接安装；② 未收录时走 AUR 源码构建（仓库 `zig` 0.16.0 + `pandoc-bin` + `ghostty-git`：sed 移除过时的 `zig<0.16.0` makedepend 与 `pandoc-cli`，约增加 25-50 分钟构建时间）；③ 任一环节失败回退 kitty。niri 终端快捷键（Mod+T）通过 `spawn-sh` 运行时探测，两个终端任一存在即可用。
