...
while getopts "i:v:K:L:P:a:b:c:d:e:f:g:h:j:n:S:t:u:A:T:R:" opt; do
  case $opt in
    ...
    T) TERMINAL="$OPTARG" ;; # 终端选择（kitty/ghostty）
    R) REMOTE="$OPTARG" ;; # 远程访问（none/wayvnc/lamco）
    *) echo "用法: $0 -i <template.Dockerfile> [-v <version>] [-S <true|false>] [-T <kitty|ghostty>]" ; exit 1 ;;
  esac
done
...
FINAL_NAME="${PREFIX}-${DISPLAY_BACKEND}-Droidspaces-rootfs-${ARCH}-${VERSION}-${TERMINAL}.tar.xz"
