...
usage() {
    cat <<'EOF'
Usage: build_rootfs-native.sh [options]
...
[[ -f "$DOCKERFILE" ]] || {
    printf 'Error: active Dockerfile not found: %s\n' "$DOCKERFILE" >&2
    exit 1
}
...
[[ "$VERSION" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || {
    printf 'Error: VERSION must contain only safe filename characters.\n' >&2
    exit 1
}
...
for value in \
    ...
    "$ENABLE_FIRMWARE" "$ENABLE_BINFMT" "$ENABLE_CONTAINER_INTEGRATION" \
    "$ENABLE_8GEN2_WAYLAND" "$ENABLE_DEV_TOOLS" "$ENABLE_COMPRESSION_TOOLS" \
    "$ENABLE_DOCKER" "$ENABLE_TMOE"
do
    is_bool "$value" || {
        printf 'Error: boolean options must be true or false (got %s).\n' "$value" >&2
        exit 1
    }
done
...
case "$(uname -m)" in
    aarch64|arm64) ;;
    *)
        printf 'Error: native builds require an ARM64 host; use build_rootfs-qemu-aarch64.sh on x86_64.\n' >&2
        exit 1
        ;;
esac
