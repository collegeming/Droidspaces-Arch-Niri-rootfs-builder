...
- The terminal is chosen via the `terminal` workflow input: **`kitty` (default)** installs straight from the repo, stable and fast; **`ghostty`** builds Arch's official stable 1.3.1 PKGBUILD from source (~25-50 min, using the official zig 0.16.0 binary + online fetch to work around the `--system` offline manifest bug; ghostty build failure is a hard failure, no silent kitty fallback). The niri terminal shortcut (Mod+T) detects the installed terminal at runtime via `spawn-sh`.
