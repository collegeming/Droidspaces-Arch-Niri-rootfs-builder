...
- The terminal prefers **ghostty** (three-tier fallback): ① install straight from the ALARM repository once it carries ghostty; ② otherwise build from Arch's official **stable 1.3.1 PKGBUILD** (repo `zig` 0.16.0 satisfies its unconstrained makedepend; only `pandoc-cli` is sed-removed and an official aarch64 pandoc binary is installed to generate man pages; adds roughly 25-50 minutes of build time); ③ fall back to kitty if any step fails. The niri terminal shortcut (Mod+T) detects at runtime via `spawn-sh`, so either terminal works.
