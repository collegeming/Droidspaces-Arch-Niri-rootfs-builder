...
- The terminal prefers **ghostty** (three-tier fallback): ① install straight from the ALARM repository once it carries ghostty; ② otherwise build from AUR (`zig0.15-bin` + `pandoc-bin` + `ghostty-git`: the `zig<0.16.0` makedepend is removed via sed and a PATH shim points zig at zig-0.15; adds roughly 25-50 minutes of build time); ③ fall back to kitty if any step fails. The niri terminal shortcut (Mod+T) detects at runtime via `spawn-sh`, so either terminal works.
