...
- The terminal prefers **ghostty** (three-tier fallback): ① install straight from the ALARM repository once it carries ghostty; ② otherwise build from AUR (repo `zig` 0.16.0 + `pandoc-bin` + `ghostty-git`: the stale `zig<0.16.0` makedepend and `pandoc-cli` are removed via sed; adds roughly 25-50 minutes of build time); ③ fall back to kitty if any step fails. The niri terminal shortcut (Mod+T) detects at runtime via `spawn-sh`, so either terminal works.
